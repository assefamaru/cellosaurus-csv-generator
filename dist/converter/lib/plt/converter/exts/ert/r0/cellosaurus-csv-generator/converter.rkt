...
(define (activate in)
  (printf "\"\",")
  (print-lst '("id" "ac" "as" "sy" "dr" "rx" "ww" "cc" "st" "di" "ox" "hi" "oi" "sx" "ca"))
  (iter in))
...
(define cwd-str (list->string (drop-right (string->list (path->string cwd)) 1)))
(define input-path (string-append cwd-str "cellosaurus.txt"))
(define output-path (string-append cwd-str "cellosaurus.csv"))
